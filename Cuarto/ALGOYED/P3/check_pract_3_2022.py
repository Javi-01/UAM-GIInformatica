...
def main(size, shift):
    ...
    for k in range(size + 2 * shift):
        print(" qsel {0:d}: {1:s}\t{2:s}".format(
            k-shift, str(p3.qsel(t, k-shift)), str(p3.qsel_nr(t, k-shift))))

    _ = input("\npulsar Intro para continuar " + 20*'.' + "\n")

    print(30*'_' + "Checking QSel5")
    for k in range(size + 2 * shift):
        print("qsel5 {0:d}: {1:s}".format(
            k-shift, str(p3.qsel5_nr(t, k-shift))))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-sz", "--size", help="size of perms for qsel", type=int, default=10)
    parser.add_argument(
        "-sh", "--shift", help="size of shift for qsel", type=int, default=3)
