...
class ValidacionSimple(EstrategiaParticionado):
    ...
    def creaParticiones(self, datos, seed=None):
        ...
        for _ in range(0, self.numeroEjecuciones):
            ...
            proporcionTrain = 1 - (self.proporcionTest / 100)

            part.indicesTrain.extend(
                indices[0: int(np.floor(proporcionTrain * filas))])
            part.indicesTest.extend(
                indices[int(np.floor(proporcionTrain * filas)): filas])
