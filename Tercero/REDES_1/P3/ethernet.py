...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    if macDest != broadcastAddr and macDest != macAddress: 
        return

    if str(ethertype) not in upperProtos.values():
        return

    for k, v in upperProtos.items():
        if v == str(ethertype):
            key = k

    key(us, header, data[14:], macOrig) # Enviamos el paquete del nivel de red sin la cabecera del nivel de enlace
...
def registerCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    # donde la clave es la función de callback y el valor el ethertype asocidado 
    upperProtos[callback_func] = str(struct.pack('!H', ethertype))    
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    ...
    # En caso de no tener el tamaño mínimo, rellenamos la cabecera
    # En vez de hasta 60 es hasta 46 porq restamos la cabecera de la trama Ethernet
    if length < 46:
