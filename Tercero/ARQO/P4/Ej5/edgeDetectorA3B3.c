...
{
    int width, height, nchannels, nproc = 0;
    ...
    // Establecemos los hilos a usar
    nproc = omp_get_num_procs();
    printf("Se usan %d hilos\n", nproc);
...
#define PIXEL_EDGES(x, y) (edges[(x) + (y)*width_edges]);
        ...
        if (TYPE_FILTER == MEDIAN)
        {
            ...
            #pragma omp parallel for firstprivate(x, y, k, width_edges, height_edges, radius, array, edges_denoised, width_denoised)
        ...
        // More classic gaussian filter
            ...
            double sum = 0;
            #pragma omp parallel for firstprivate(x, y, k, width_edges, height_edges, radius, sum, kernel, edges, array, edges_denoised, width_denoised)
            for (int i = radius; i < width_edges - radius; i++)
            {
                for (int j = radius; j < height_edges - radius; j++)
                {
